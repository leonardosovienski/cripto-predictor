...
ROOT = Path(os.environ.get("CRIPTO_SOURCE_ROOT", Path(__file__).resolve().parents[2]))
...
TRIALS_PATH = Path(
    os.environ.get("CRIPTO_TRIALS_PATH", ROOT / "GarimpoInvestimentos" / "trials.json")
)
...
def main() -> int:
    ...
    if not args.dry_run and not (ROOT / ".git").exists():
        ap.error(
            "attestation requires an explicit clean CRIPTO_SOURCE_ROOT; wheel installation is not a source checkout"
        )
