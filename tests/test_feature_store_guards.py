...
def test_teto_e_ajustavel_por_instancia(tmp_path):
    ...
    with FeatureStore(tmp_path / "fs.db", max_publication_lag=timedelta(days=400)) as s:
...
def test_versoes_de_feature_coexistem_sem_sobrescrever(store):
    ...
    store.write_features("bitcoin", "1d", [{"ts": TS, "rsi": 55.0}], feature_version="v2")
...
def test_serving_default_le_v1(store):
    store.write_features("bitcoin", "1d", [{"ts": TS, "rsi": 30.0}])
    store.write_features("bitcoin", "1d", [{"ts": TS, "rsi": 99.0}], feature_version="v2")
