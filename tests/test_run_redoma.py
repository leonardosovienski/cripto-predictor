...
def test_run_analisa_do_serving_e_persiste_carimbado(tmp_path, monkeypatch):
    ...
    with FeatureStore(db) as fs:
        fs.write_raw([MarketDataPoint(
            source="coingecko", symbol="bitcoin", interval="1d", timestamp=ts,
            open=1.0, high=2.0, low=0.5, close=60000.0, volume=1e9, published_at=ts)])
        fs.write_features("bitcoin", "1d",
                          [{"ts": ts, "price_usd": 60000.0, "close": 60000.0, "rsi_14": 40.0}])
    ...
    assert len(preds) == 1, "run() terminou sem persistir a previsão (regressão do close prematuro?)"
    p = preds[0]
