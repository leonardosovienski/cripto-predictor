...
def test_martj42_parse_canonicaliza(tmp_path):
    ...
    csv = ("date,home_team,away_team,home_score,away_score,tournament\n"
...
def _match(day, mid):
    ...
    return MatchObservation("martj42", mid, ts, "brazil", "argentina",
...
def test_event_align_so_usa_info_antes_do_kickoff():
    ...
    rk = [SignalPoint("rank", datetime(2026, 6, 5, tzinfo=UTC), 1.0, "fifa",
                      datetime(2026, 6, 5, tzinfo=UTC)),
          SignalPoint("rank", datetime(2026, 6, 15, tzinfo=UTC), 2.0, "fifa",
...
def test_event_align_nan_se_nada_publicado_antes():
    matches = [_match(1, "m0")]
    rk = [SignalPoint("rank", datetime(2026, 6, 5, tzinfo=UTC), 1.0, "fifa",
                      datetime(2026, 6, 5, tzinfo=UTC))]  # publicado DEPOIS do jogo
    rows = EventAlignmentEngine().align(matches, {"rank": rk})
