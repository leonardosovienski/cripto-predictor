...
def _cotahist_line(date="20260102", codbdi="02", ticker="PETR4", tpmerc="010",
                   o=3050, h=3100, lo=3000, c=3080, vol=10000000) -> str:
...
def test_cotahist_provider_fetch(tmp_path):
    ...
    f.write_text("\n".join([_cotahist_line(date="20260102"),
                            _cotahist_line(date="20260103", c=3120)]), encoding="latin-1")
...
def _candle(day, close):
    ...
    return MarketDataPoint(symbol="PETR4", timestamp=ts, open=close, high=close,
                           low=close, close=close, volume=0, source="cotahist",
                           interval="1d", published_at=ts + timedelta(hours=18))
...
def test_alignment_point_in_time_nao_usa_revisao_futura():
    ...
    v1 = SignalPoint("ipca", ref, 0.40, "bcb_sgs", datetime(2026, 4, 10, tzinfo=UTC),
                     reference_date=ref, vintage=datetime(2026, 4, 10, tzinfo=UTC))
    v2 = SignalPoint("ipca", ref, 0.43, "bcb_sgs", datetime(2026, 5, 15, tzinfo=UTC),
                     reference_date=ref, vintage=datetime(2026, 5, 15, tzinfo=UTC))
...
def test_bcb_provider_parse_e_published_at(monkeypatch):
    ...
    class _Resp:
        ...
        def json(self): return [{"data": "31/03/2026", "valor": "0.40"}]
...
def test_ingest_stocks_materializa_e_registra_proveniencia(tmp_path, monkeypatch):
    ...
    f = tmp_path / "COTAHIST.TXT"
    f.write_text("\n".join([_cotahist_line(date="20260102", c=3050),
                            _cotahist_line(date="20260103", c=3120)]), encoding="latin-1")
    ...
    class _FakeSelic(SignalProvider):
        ...
        async def fetch(self, limit=30, **kw):
            ...
            return [SignalPoint("selic", ts, 11.25, "bcb_sgs", ts,
                                reference_date=ts, vintage=ts)]

    facade = StocksDataProvider.from_cotahist(f, signal_providers=[_FakeSelic()])
    with FeatureStore(tmp_path / "fs.db") as fs:
        aligned = asyncio.run(ingest_stocks(fs, facade, "PETR4", limit=10,
