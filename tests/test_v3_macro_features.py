"""Legacy synthetic business-day assumption tests, NOT actual Fed availability.
...
class TestMacroEventDummy:
    def test_dentro_da_janela_marca_1(self):
        ...
        out = build_macro_event_dummy(
            vectors, window_days=1, events=events, assume_calendar_known=True
        )
    ...
    def test_combina_tipos_com_or(self):
        ...
        out = build_macro_event_dummy(
            vectors, window_days=0, events=events, assume_calendar_known=True
        )
        assert out == [1.0, 1.0, 0.0]

    def test_window_days_negativo_e_erro(self):
        with pytest.raises(ValueError, match="negativo"):
            build_macro_event_dummy([], window_days=-1, events=[], assume_calendar_known=True)
...
class TestDxyReturn:
    def test_retorno_calculado_com_lag(self):
        ...
        out = build_dxy_return(vectors, closes, assume_business_day_lag=True, publish_lag_days=1)
    ...
    def test_cobertura_insuficiente_retorna_zero_neutro(self):
        ...
        out = build_dxy_return(
            vectors, {date(2026, 9, 8): 102.0}, assume_business_day_lag=True, publish_lag_days=1
        )
        assert out == [0.0]

    def test_publish_lag_negativo_e_erro(self):
        with pytest.raises(ValueError, match="negativo"):
            build_dxy_return([], {}, assume_business_day_lag=True, publish_lag_days=-1)

    def test_fim_de_semana_nao_usa_close_de_sexta_ainda_nao_publicado(self):
        """Achado de auditoria externa (2026-09-05): com assume_business_day_lag=True, publish_lag_days=1 dia
        ...
        sabado = build_dxy_return(
            [_fv(date(2026, 9, 5))], closes, assume_business_day_lag=True, publish_lag_days=1
        )
        domingo = build_dxy_return(
            [_fv(date(2026, 9, 6))], closes, assume_business_day_lag=True, publish_lag_days=1
        )
    ...
    def test_segunda_ja_pode_usar_close_de_sexta(self):
        ...
        segunda = build_dxy_return(
            [_fv(date(2026, 9, 7))], closes, assume_business_day_lag=True, publish_lag_days=1
        )
...
class TestDxyLookaheadDiasUteis:
    ...
    def test_fim_de_semana_nao_enxerga_o_close_de_sexta(self, dia):
        ...
        out = build_dxy_return(
            [_fv(dia)], self.CLOSES, assume_business_day_lag=True, publish_lag_days=1
        )
    ...
    def test_segunda_ja_enxerga_o_close_de_sexta(self):
        ...
        out = build_dxy_return(
            [_fv(date(2026, 9, 7))], self.CLOSES, assume_business_day_lag=True, publish_lag_days=1
        )
        assert out == pytest.approx([self._ret_de_sexta()])

    def test_lag_zero_nao_antecipa_publicacao(self):
        """Com lag=0 o close do próprio dia é conhecido no dia — mas nunca antes."""
        assert build_dxy_return(
            [_fv(date(2026, 9, 4))], self.CLOSES, assume_business_day_lag=True, publish_lag_days=0
        ) == (pytest.approx([self._ret_de_sexta()]))
        assert build_dxy_return(
            [_fv(date(2026, 9, 3))], self.CLOSES, assume_business_day_lag=True, publish_lag_days=0
...
class TestDxyCoverage:
    def test_conta_pontos_imputados(self):
        ...
        imputados, total = dxy_coverage(
            vectors, closes, assume_business_day_lag=True, publish_lag_days=1
        )
    ...
    def test_serie_vazia_imputa_tudo(self):
        ...
        assert dxy_coverage(vectors, {}, assume_business_day_lag=True, publish_lag_days=1) == (1, 1)

    def test_lag_negativo_e_erro(self):
        with pytest.raises(ValueError, match="negativo"):
            dxy_coverage([], {}, assume_business_day_lag=True, publish_lag_days=-1)
