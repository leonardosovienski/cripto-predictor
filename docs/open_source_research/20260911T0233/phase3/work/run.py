...
OUT=pathlib.Path(sys.argv[1]) if len(sys.argv)>1 else R
OUT.mkdir(parents=True,exist_ok=True)
def write(n,x):(OUT/n).write_text(json.dumps(x,indent=2,ensure_ascii=False),encoding='utf-8')
def rejects(fn):
 try:fn();return False
...
for c in b['sample']:
 ...
 if t is not None:
  proven=[f['value'] for k,f in e['fields'].items() if k in ['INGESTION_TIME','PUBLICATION_TIME','REVISION_TIME','EVENT_TIME'] and f['status']=='PROVEN']
  boundary=max(proven)
  temporal=[{'cutoff_ns':t-1,'eligible':a.available(e,t-1),'expected':False},{'cutoff_ns':t,'eligible':a.available(e,t),'expected':all(v<=t for v in proven)},{'cutoff_ns':boundary,'eligible':a.available(e,boundary),'expected':True}]
...
summary={'TOTAL_CASES':len(records),'REAL_HTTP_RPC_RECEIPTS':sum(c['kind']!='mapping' for c in b['sample']),'RETROSPECTIVE_ARTIFACTS':1,'VALID':0,'CORRECTLY_REJECTED':sum(r['classification']=='CORRECTLY_REJECTED' for r in records),'AMBIGUOUS':sum(r['classification']=='AMBIGUOUS' for r in records),'UNSUPPORTED':sum(r['classification']=='UNSUPPORTED' for r in records),'BUGS_FOUND':1,'BUGS_FIXED':1,'BUG_SCOPE':'test oracle only; zero demonstrated v2 production bugs','REPRESENTATION_GAPS_ADDRESSED':2,'TEMPORAL_VIOLATIONS':violations,'SOURCE_CLOCK_INCONSISTENCIES':1,'SILENT_INFERENCES':0,'ROUND_TRIP_FAILURES':sum(not r['round_trip'] for r in records),'FAITHFUL_PARTIAL_ENVELOPES':len(records),'COMPLETE_ECONOMIC_IDENTITIES':0,'ADVERSARIAL_TESTS':len(tests),'ADVERSARIAL_REJECTED':sum(t['rejected'] for t in tests),'TEMPORAL_CHECKS':sum(len(r['temporal']) for r in records)}
