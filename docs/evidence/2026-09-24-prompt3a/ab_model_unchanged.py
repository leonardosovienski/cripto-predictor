...
with (sym / "funding.csv").open("w", encoding="utf-8") as f:
    ...
    for i in range(days * 3):
        ...
        f.write(
            f"BTCUSDT,{START + i * 8 * HOUR},{rng.gauss(0, 0.00005) + spike:.8f},{30000.0 + 500.0 * math.sin(i / 13.0):.2f}\n"
        )
...
with (sym / "spot_binance_1h.csv").open("w", encoding="utf-8") as f:
    ...
    for i in range(days * 24):
        f.write(
            f"BTCUSDT,{START + i * HOUR},{30000.0 + 500.0 * math.sin(i / 104.0) + 40.0 * math.sin(i / 3.0):.2f},100.0\n"
        )
...
summary["folds"] = [
    {k: v for k, v in asdict(f).items() if k not in ("is_last_train_ms", "oos_first_test_ms")}
    for f in r.folds
]
